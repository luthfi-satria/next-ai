type listComponentProps<T extends Record<string, unknown>> = {
  title: string
  listObj: T
}

export default function ListComponent<T extends Record<string, unknown>>({
  title,
  listObj,
  ...
  return (
    <div className="bg-white rounded-lg shadow-xl p-6 mb-8 border border-gray-200">
      ...
      {Object.keys(listObj).map((keyname) => (
        <div
          key={`list${keyname}`}
          className="mb-5 pb-4 border-b border-gray-100 last:mb-0 last:border-b-0"
        >
              ...
              <div className="text-white bg-green-700 rounded-md p-1 w-full grow text-center">
                {typeof listObj[keyname] == "boolean"
                  ? listObj[keyname] == false
                    ? "No"
                    : "Yes"
                  : typeof listObj[keyname] == "number"
                    ? listObj[keyname].toFixed(2)
                    : String(listObj[keyname])}
              </div>
            </div>
          )}
        </div>
