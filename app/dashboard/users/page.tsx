...
export default function HomePage() {
  ...
  const fetchUsers = async () => {
    ...
    setError(null)
    try {
      const { response, ApiResponse } = await PopulateTable('/api/users', currentFilters, currentPage, limit)
  ...
  const handleDelete = async () => {
    if (selectedUser) {
      try {
        const { response, data } = await PUSHAPI('DELETE', `/api/users/${selectedUser}`, '')
