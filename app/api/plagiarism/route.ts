...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json(
    { message: "This is the plagiarism tools API endpoint. Please use GET." },
    { status: 200 },
  )
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const uploadedFormidableFiles: FormidableFile[] = files.files || []
    ...
    const plagiarismResults: object[] = []

    for (const file of uploadedFormidableFiles) {
      const filePath = file.filepath
      const mimeType = file.mimetype || "application/octet-stream"
      let fileResult: object = { fileName: file.originalFilename }
    ...
    return NextResponse.json(
      { success: false, error: "Internal error occur" },
      { status: 500 },
    )
  }
}
