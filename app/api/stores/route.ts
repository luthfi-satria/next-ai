...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!newCollection.name) { 
            return NextResponse.json({ success: false, message: 'Store name are required' }, { status: 400 })
        }
        ...
        return NextResponse.json({ success: true, data: result.insertedId, message: 'Stores created successfully' }, { status: 201 })
    } catch (error: any) {
