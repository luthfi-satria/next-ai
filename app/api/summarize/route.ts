...
export async function GET() {
  return NextResponse.json({
    message: "This is the summarize API endpoint. Please use POST.",
  })
}
...
export async function POST(req: Request) {
  try {
    ...
    if (typeof textToSummarize !== "string" || textToSummarize.trim() === "") {
      return NextResponse.json(
        { error: "Text to summarize is empty or invalid." },
        { status: 400 },
      )
    }
    ...
    return NextResponse.json(
      {
        error: `Failed to summarize text: ${errorMsg || "Internal server error"}`,
      },
      { status: 500 },
    )
  }
