...
export default class MultipleUploadLibrary {
  ...
  constructor(config: uploadConfig) {
    ...
    this.config.maxFileSize =
      (this.config?.maxFileSize ?? MAX_FILE_SIZE) * 1024 * 1024
    ...
    this.resolvedUploadDir = path.join(
      process.cwd(),
      DEFAULT_UPLOAD_DIR,
  ...
  async parse(req: IncomingMessage): Promise<UploadResponse> {
    ...
    const form = new IncomingForm({
      ...
      filter: ({ originalFilename, mimetype }) => {
        ...
        if (!allowed) {
          console.log(
            `File ${originalFilename} with type ${mimetype} is not allowed.`,
          )
        }
        return allowed
      },
      ...
      form.parse(req, (err, fields, files) => {
        if (err) {
          if (err.code === formidable?.errors?.biggerThanMaxFileSize) {
            return reject(
              new Error(
                `File size limit exceeded: ${this.config.maxFileSize / (1024 * 1024)}MB per file`,
              ),
            )
          }
          ...
          if (
            err.message &&
            err.message.includes("Maximum number of files exceeded")
          ) {
            return reject(
              new Error(
                `Too many files uploaded. Max: ${this.config.maxNumberOfFiles}`,
              ),
            )
          }
        ...
        for (const key in files) {
          if (Object.prototype.hasOwnProperty.call(files, key)) {
            ...
            if (fileValue) {
              normalizedFiles[key] = Array.isArray(fileValue)
                ? (fileValue as FormidableFile[])
                : ([fileValue] as FormidableFile[])
            }
          }
        }
    ...
    for (const file of files) {
      ...
      if (unlinking === true) {
        try {
          ...
          console.error(
            `file temp failed to removed ${file.filepath}:`,
            unlinkErr,
          )
        }
      }
